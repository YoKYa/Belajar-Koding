						...
						<div class="block w-full overflow-x-auto">
							...
							<div class="flex">
								<input
									...
									placeholder="Quote (Max 100 Huruf)"
									required
									maxlength="100"
									v-model="form.quote"
									v-bind:class="{
					...
					>
						...
						<div class="mt-2">
							<p class="text-sm text-gray-500">
								Apakah kamu yakin menghapus Quote
...
export default {
	...
	setup() {
		...
		const createQuote = async () => {
			if (form.title == "" && form.quote == "") {
				...
				try {
					...
					await axios.post("api/quotes/create", form);
					loading.text = true;
					setTimeout(() => {
						loading.load = false;
						form.title = "";
		...
		const deleteQuoteStore = async () => {
			try {
				deleteOpen.value = false;
				loading.load = true;
				loading.text = false;
				await axios.get("sanctum/csrf-cookie");
				await axios.post("api/quotes/delete", { id: form.id });
				loading.text = true;
				setTimeout(() => {
					loading.load = false;
					loading.text = false;
		...
		const updateQuote = async () => {
			if (form.title == "") {
				...
				try {
					...
					loading.text = false;
					await axios.get("sanctum/csrf-cookie");
					await axios.post("api/quotes/store", form);
					loading.text = true;
					setTimeout(() => {
						loading.load = false;
						loading.text = false;
