					...
					>
						...
						<div class="mt-2">
							<p class="text-sm text-gray-500">
								Apakah kamu yakin menghapus Permission
	...
	setup() {
		...
		const createPermission = async () => {
			if (form.name == "") {
				...
				try {
					...
					await axios.post("api/permissions/create", form);
					loading.text = true;
					setTimeout(() => {
						loading.load = false;
						form.name = "";
		...
		const editPermission = async (id) => {
			try {
				...
				loading.text = false;
				await axios.get("sanctum/csrf-cookie");
				let response = await axios.post("api/permissions/edit", {
					id: id,
				});
		...
		const deletePermissionStore = async () => {
			try {
				deleteOpen.value = false;
				loading.load = true;
				loading.text = false;
				await axios.get("sanctum/csrf-cookie");
				await axios.post("api/permissions/delete", { id: form.id });
				loading.text = true;
				setTimeout(() => {
					loading.load = false;
					loading.text = false;
		...
		const updatePermission = async () => {
			if (form.name == "") {
				...
				try {
					...
					loading.text = false;
					await axios.get("sanctum/csrf-cookie");
					await axios.post("api/permissions/update", form);
					loading.text = true;
					setTimeout(() => {
						loading.load = false;
						loading.text = false;
